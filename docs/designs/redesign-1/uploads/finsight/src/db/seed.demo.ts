...
async function seedDemo() {
  ...
  if (RESET) {
    ...
    await db
      .delete(transactions)
      .where(
        inArray(transactions.accountId, [
          DEMO_CURRENT_ACCOUNT_ID,
          DEMO_SAVINGS_ACCOUNT_ID,
        ]),
      );
    await db
      .delete(accounts)
      .where(eq(accounts.connectionId, DEMO_CONNECTION_ID));
    await db
      .delete(bankConnections)
      .where(eq(bankConnections.id, DEMO_CONNECTION_ID));
  ...
  for (const cat of CATEGORIES) {
    ...
    await db
      ...
      .values({
        ...rest,
        isDefault: true,
        groupId: group ? groupMap[group] : null,
      })
      .onConflictDoNothing();
  }
  ...
  for (const d of salaryDates) {
    ...
    addTx(
      ...
      null,
      "ACME CORP LTD",
      `SALARY ${d.toLocaleString("en-IE", { month: "short" }).toUpperCase()} ACME CORP`,
      "Income",
      "rule",
    );
  }
  ...
  for (const d of dates) {
    ...
    if (
      (dow === 2 && Math.random() < 0.7) ||
      (dow === 4 && Math.random() < 0.6) ||
      (dow === 6 && Math.random() < 0.85)
    ) {
  ...
  for (const d of dates) {
    ...
    if (Math.random() < 0.08) {
      ...
      const amount = -parseFloat(
        jitter(payee === "UBER" ? 12 : 28, 0.5).toFixed(2),
      );
      addTx(
        DEMO_CURRENT_ACCOUNT_ID,
        d,
  ...
  for (const month of holidayMonths) {
    ...
    if (d <= END_DATE) {
      addTx(
        DEMO_CURRENT_ACCOUNT_ID,
        d,
        -parseFloat(jitter(480, 0.4).toFixed(2)),
        "BOOKING.COM",
        null,
      ...
      addTx(
        DEMO_CURRENT_ACCOUNT_ID,
        addDays(d, 1),
        -parseFloat(jitter(220, 0.4).toFixed(2)),
        "RYANAIR",
        null,
  ...
  for (const m of interestMonths) {
    ...
    if (d <= END_DATE) {
      addTx(
        DEMO_SAVINGS_ACCOUNT_ID,
        d,
        parseFloat(jitter(12, 0.5).toFixed(2)),
        null,
        null,
        "INTEREST PAYMENT",
        "Income",
