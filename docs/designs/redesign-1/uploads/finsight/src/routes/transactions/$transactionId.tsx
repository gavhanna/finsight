...
function TransactionDetailPage() {
	...
	const merchantChartData =
		transaction.merchant?.monthlySpend.map((entry) => ({
			...entry,
	...
	const merchantInsightCardsRaw: Array<MerchantInsightCard | null> = [
		transaction.merchantContext?.recurring
			? {
					...
					tone: transaction.merchantContext.recurring.isActive
						? "primary"
						: "default",
					icon: Repeat2,
				}
		...
		<div className="p-4 md:p-6 space-y-5">
						...
						<div className="space-y-2">
							...
							<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
								...
								{transaction.valueDate && (
									<span>Value date {formatDate(transaction.valueDate)}</span>
								)}
								<span>
									Account{" "}
			...
			<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
				...
				<SummaryCard
					...
					secondary={
						transaction.connection?.institutionName ??
				...
				<SummaryCard
					...
					label="Institution"
					value={
						transaction.connection?.institutionName ?? "Unknown institution"
					}
				...
				<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
					{transaction.categoryContext.budgetedAmount && (
						<InsightCard
							title="Budget Impact"
							value={formatPercent(
								transaction.categoryContext.transactionShareOfBudget,
							)}
					...
					<InsightCard
						...
						value={formatPercent(
							transaction.categoryContext.transactionShareOfMonth,
						)}
				...
				<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
					...
					<SummaryCard
						...
						value={formatCurrency(
							transaction.merchant.totalSpend,
					...
					<SummaryCard
						...
						label="Typical spend"
						value={formatCurrency(
							transaction.merchant.averageSpend,
							transaction.currency,
						)}
					/>
				</div>
					...
					<CardContent className="space-y-5">
						<div className="grid gap-4 md:grid-cols-2">
							...
							<Field label="Merchant category code">
								<span>
									{transaction.merchantCategoryCode ?? "Not provided"}
								</span>
							</Field>
							...
							{merchantChartData.length > 1 && (
								<Card>
									<CardHeader>
										...
										<CardDescription>
											A quick view of how this merchant has shown up in your
											spending over time.
										</CardDescription>
									</CardHeader>
									<CardContent className="pb-4">
										<ResponsiveContainer width="100%" height={180}>
											<BarChart data={merchantChartData} barSize={24}>
												...
												<Tooltip
													content={
														<ChartTooltip currency={transaction.currency} />
													}
												/>
												<Bar
													dataKey="total"
									...
									{transaction.merchant.recentTransactions.map((merchantTx) => {
										...
										return (
											<Link
												...
												params={{
													transactionId: encodeURIComponent(merchantTx.id),
												}}
												search={search}
												className={cn(
											...
											>
												<div className="min-w-0">
													<div className="flex items-center gap-2">
														...
														{isCurrent && (
															<Badge variant="outline">This transaction</Badge>
														)}
													</div>
													<p className="truncate text-sm text-muted-foreground">
														{merchantTx.description ??
															transaction.merchant?.canonicalName}
													</p>
												</div>
							...
							<CardContent className="grid gap-4">
								<Field label="Account IBAN">
									<span className="break-all">
										{transaction.account.iban ?? "Not provided"}
									</span>
								</Field>
								<Field label="Account owner">
									<span>{transaction.account.ownerName ?? "Not provided"}</span>
								...
								<Field label="Institution status">
									<span>
										{transaction.connection?.status ?? "Not provided"}
									</span>
								</Field>
							</CardContent>
						</Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Expand this when you need the original bank transaction object for
						debugging or support.
					</CardDescription>
				</CardHeader>
				<CardContent>
					{transaction.rawDataText ? (
						...
						<p className="text-sm text-muted-foreground">
							No raw payload was stored for this transaction.
						</p>
					)}
				</CardContent>
			</Card>
